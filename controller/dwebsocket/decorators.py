...
WEBSOCKET_MIDDLEWARE_INSTALLED = 'controller.dwebsocket.middleware.WebSocketMiddleware' in getattr(settings,'MIDDLEWARE_CLASSES', [])


def _setup_websocket(func):
    # print('进入_setup_websocket')
    from functools import wraps
    @wraps(func)
    def new_func(request, *args, **kwargs):
        # print('进入new_func')
        response = func(request, *args, **kwargs)
        # print('输出response' + str(response))
